...
foreach ($smtp_servers as $name => $server_info) {
    $host = $server_info[0];
    $port = $server_info[1];
...
if (file_exists('vendor/autoload.php')) {
    ...
    // Создаем mail объект для тестирования
    $mail_class_exists = class_exists('PHPMailer\\PHPMailer\\PHPMailer');
    if ($mail_class_exists) {
        echo "   ✓ PHPMailer классы загружены\n";
        
        // Простой тест подключения без use statements
        try {
        ...
        echo "   ✗ PHPMailer классы не найдены\n";
    }
} else {
    echo "   - PHPMailer не найден (vendor/autoload.php отсутствует)\n";
}
