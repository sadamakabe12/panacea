<?php
// doctor/export_pdf.php - Экспорт медицинской записи в PDF из интерфейса врача
...
// Проверяем ID записи
$record_id = isset($_GET['record_id']) ? intval($_GET['record_id']) : (isset($_GET['id']) ? intval($_GET['id']) : 0);
if (!$record_id) {
...
// Проверяем, принадлежит ли запись текущему врачу или это запись его пациента
if ($medical_record['doctor_id'] != $userid) {
    // Проверяем, является ли пациент пациентом текущего врача
    $is_doctor_patient = check_doctor_patient_relationship($database, $userid, $medical_record['patient_id']);
    if (!$is_doctor_patient) {
        die("У вас нет доступа к этой медицинской записи");
    }
}
...
log_medical_record_action($database, $userid, 'doctor', $record_id, 'export', $action_details);
...
require_once('../admin/pdf_generator.php');
...
$pdf->addFooter("Отчет сформирован: " . date('d.m.Y H:i:s') . " | Врач: " . $medical_record['doctor_name']);
