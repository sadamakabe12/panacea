...
Тестирование разработанной медицинской информационной системы "Панацея" произведено с помощью сквозного (end-to-end) тестирования, позволившего проверить все сценарии вариантов использования МИС на корректность и отсутствие неожиданного поведения. Были протестированы как позитивные, так и негативные сценарии каждого варианта использования.
...
**Цель теста:** проверить функциональность восстановления забытого пароля пользователя.

**Шаги:**
1) открыть страницу авторизации;
2) нажать ссылку «Забыли пароль?»;
3) ввести почту зарегистрированного пользователя;
4) нажать кнопку «Отправить ссылку для сброса».

**Ожидаемый результат:** система отправляет на указанную почту ссылку для сброса пароля и отображает сообщение о успешной отправке.

**Другие возможные исходы:** при вводе незарегистрированной почты система отображает сообщение "Пользователь с такой почтой не найден" (см. рисунок 4.5).

**Варианты проверки:**
1) восстановление пароля для существующего пользователя;
2) попытка восстановления для несуществующей почты;
3) переход по ссылке сброса и создание нового пароля.

*Рисунок 4.5 – Сообщение при восстановлении пароля*
3) заполнить все поля формы регистрации (почта, имя, пароль, дата рождения, телефон);
4) принять условия использования;
5) нажать кнопку «Зарегистрироваться».

**Ожидаемый результат:** система создаст учётную запись пациента и отправит письмо с подтверждением на указанную почту.

**Другие возможные исходы:** если заполнить не все поля формы или заполнить неверно (почта уже используется), то система выведет сообщение с описанием ошибки (рис. 4.2-4.3).

**Варианты проверки:**
1) при регистрации заполнить не все поля формы или заполнить неправильно;
2) использовать уже зарегистрированную почту.
...
## 4.1.35 Просмотр всех записей на приём
...
## 4.1.37 Просмотр журнала аудита
...
## 4.1.38 Управление специальностями
...
## 4.1.39 Создание детального расписания врача
...
## 4.1.40 Навигация по неделям в расписании
...
## 4.1.41 Управление уведомлениями о записях
...
## 4.1.42 Групповая отмена записей
...
## 4.1.43 Экспорт данных для отчётности
...
## 4.1.45 Резервное копирование и восстановление
